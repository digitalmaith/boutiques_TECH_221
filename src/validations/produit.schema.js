...
export const createProduitSchema = z.object({
  ...
  prix: z.coerce.number().positive('Le prix doit être positif').min(0.01, 'Le prix minimum est de 0.01'),
  qteStock: z.coerce.number().int('La quantité doit être un entier').min(0, 'La quantité ne peut pas être négative'),
  categorieId: z.coerce.number().int().positive('La catégorie est obligatoire'),
